...
public class UserActiveSubscriptionsCommandTest {
    ...
    @InjectMocks
    private UserActiveSubscriptionsCommand userActiveSubscriptionsCommand;

    @Test
    public void execute() throws CommandExeption {
        when(request.getSession()).thenReturn(session);
        when(session.getAttribute(SESSION_ATTR_USER_ID)).thenReturn(1);
        assertNotNull(userActiveSubscriptionsCommand.execute(request, response));
    }
}