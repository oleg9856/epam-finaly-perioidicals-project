...
public class UserBalanceOperationCommandTest {
    ...
    @Mock
    private BalanceOperationService balanceOperation  ;

    @InjectMocks
    private UserBalanceOperationCommand userBalanceOperationCommand;

    @Test
    public void testSuccessExecute() throws CommandExeption {
        when(request.getSession()).thenReturn(session);
        when(session.getAttribute(SESSION_ATTR_USER_ID)).thenReturn(1);
        assertNotNull(userBalanceOperationCommand.execute(request, response));
    }
}